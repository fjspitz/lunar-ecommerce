...
class SearchService
{
    public function searchBy(string $customer_group, string $criteria, $value, int $page_size): ResourceCollection
    {
        $result = match ($criteria) {
            ProductSearchCriteria::BY_NAME->value =>
            ...
            ProductResource::collection(
                Product::where('product_type_id', (int)$value)
                    ->where('status', 'published')
            ...
            ProductResource::collection(
                Product::where('brand_id', (int)$value)
                    ->where('status', 'published')
                    // ->whereHas('customerGroups', function (Builder $query) use ($customer_group) {
                    //     $query->where('handle', $customer_group)
                    //         ->where('visible', true);
